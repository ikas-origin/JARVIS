...
0.3.1 新增 `read_files`，允许模型一次读取 1～8 个已知的小型相关文件；敏感路径、Git 元数据和 workspace 越界仍使用原有策略逐项拒绝。使用相同模型、提示词和 `demo-bug-start` 再运行三轮：

| 指标 | 0.3 三轮平均 | 0.3.1 三轮平均 | 变化 |
|---|---:|---:|---:|
| 成功率 | 3/3 | 3/3 | 持平 |
| Turns | 11.0 | 10.0 | -9.1% |
| Tool calls | 22.0 | 17.7 | -19.7% |
| Total tokens | 120,517 | 116,893 | -3.0% |
| Elapsed | 48.532s | 53.052s | +9.3% |

三轮分别调用 `read_files` 2、1、3 次，说明模型实际采用了新工具；最终测试分别为 15/15、14/14、14/14，`verification_status` 均为 `passed`。平均耗时没有改善，可能同时受网络和模型生成波动影响，因此这里只主张工具往返和 token 小幅下降，不主张端到端速度提升。

0.3.1 原始产物：

```text
.eval-runs/taskboard-20260831-112245/
```

下一项效率候选应针对每轮约 5～6 次 `edit_file`，但批量写入的错误影响明显高于批量读取；在引入前需要原子性、逐文件 diff 和验证门设计，不能直接复制只读批处理。
